...
def load_data(partition_id: int, num_partitions: int, split: str = "train") -> DataLoader:
    ...
    subset = dataset[split].shard(num_shards=num_partitions, index=partition_id) # type: ignore
...
def train(client_model, dataloader, device, grpc_stub, round_num=None):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        mask_arr = attention_mask.cpu().numpy() # Convert to numpy array first
        request = split_pb2.ForwardRequest( # type: ignore
            activation=activation.detach().cpu().view(-1).tolist(),
            activation_shape=list(activation.shape),
            attention_mask_bytes=mask_arr.tobytes(), # Serialize to bytes
            attention_mask_shape=list(mask_arr.shape),
            client_id=0,
...
def test(client_model, dataloader, device, grpc_stub):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            request = split_pb2.InferenceRequest( # type: ignore

...
from smpc.smpc_aggregator import encrypt_client_update
import crypten
...
class SplitLearningClient(fl.client.NumPyClient):
    def __init__(self, cid, local_epochs):
        ...
        self.channel = grpc.insecure_channel("localhost:50053", options=[
    ...
    def get_parameters(self, config=None):
        ...
        for val in self.model.state_dict().values():
            dummy = torch.zeros_like(val).cpu().numpy()
            dummy_params.append(dummy)
        return dummy_params
    ...
    def fit(self, parameters, config):
        ...
        raw_params = [val.cpu().numpy() for val in self.model.state_dict().values()]
        encrypted_update = encrypt_client_update(raw_params, scheme="additive")

        # Flatten encrypted layers and get their lengths to send to server
        flat_layers = [np.frombuffer(layer_bytes, dtype=np.uint8) for layer_bytes in encrypted_update]
        flat_encrypted_array = np.concatenate(flat_layers)
        layer_lengths = [len(layer_bytes) for layer_bytes in encrypted_update]

        log_dir = os.path.join("..", "results_smpc", "client_metrics")
        ...
        metrics = {
            ...
            "layer_lengths": json.dumps(layer_lengths),  # serialize as JSON string
        }

        return [flat_encrypted_array], total_samples, metrics
...
if __name__ == "__main__":
    ...
    fl.client.start_client(
        ...
        client=SplitLearningClient(args.cid, args.local_epochs),

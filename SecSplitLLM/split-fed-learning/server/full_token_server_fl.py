...
log_dir = os.path.join("..", "results", "server_metrics")
...
def centralized_evaluate(parameters):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # Check for invalid labels here
            if (labels < 0).any() or (labels >= 3).any():
                invalid_labels = labels[(labels < 0) | (labels >= 3)]
                raise ValueError(f"Invalid label(s) found: {invalid_labels} in batch")
...
def main():
    ...
    fl.server.start_server(
        ...
        config=fl.server.ServerConfig(num_rounds=3),  #CHANGED UNTIL FIND BEST MAX NORM
        strategy=strategy,
